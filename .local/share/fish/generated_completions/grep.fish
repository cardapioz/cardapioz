...
complete -c grep -s i -l ignore-case --description 'Ignore case distinctions in both the  PATTERN and the input files.'
...
complete -c grep -s Z -l null --description 'Output a zero byte (the \\s-1ASCII\\s0  NUL character) instead of the character…'
...
complete -c grep -l binary-files --description 'If the first few bytes of a file indicate that the file contains binary data,…'
...
complete -c grep -l exclude --description 'Skip files whose base name matches  GLOB (using wildcard matching).'
complete -c grep -l exclude-from --description 'Skip files whose base name matches any of the file-name globs read from  FILE…'
complete -c grep -l exclude-dir --description 'Exclude directories matching the pattern  DIR from recursive searches.'
...
complete -c grep -s z -l null-data --description 'Treat the input as a set of lines, each terminated by a zero byte (the ASCII …'
